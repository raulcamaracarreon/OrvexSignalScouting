from __future__ import annotations

import os
import sys
from pathlib import Path

import requests
from dotenv import load_dotenv


PROJECT_ROOT = Path(__file__).resolve().parent
ENV_PATH = PROJECT_ROOT / ".env"

DENUE_BASE_URL = (
    "https://www.inegi.org.mx/app/api/denue/v1/consulta"
)

ENTITY_CODE = "09"
MUNICIPALITY_CODE = "007"
ACTIVITY_CLASS = "468213"
STRATUM_CODE = "0"

INITIAL_RECORD = 1
FINAL_RECORD = 50


def main() -> int:
    load_dotenv(ENV_PATH)

    token = os.getenv("DENUE_TOKEN", "").strip()

    if not token:
        print("ERROR: DENUE_TOKEN no está disponible.")
        return 1

    request_url = (
        f"{DENUE_BASE_URL}/BuscarAreaActEstr/"
        f"{ENTITY_CODE}/"
        f"{MUNICIPALITY_CODE}/"
        "0/"
        "0/"
        "0/"
        "0/"
        "0/"
        "0/"
        f"{ACTIVITY_CLASS}/"
        "0/"
        f"{INITIAL_RECORD}/"
        f"{FINAL_RECORD}/"
        "0/"
        f"{STRATUM_CODE}/"
        f"{token}"
    )

    print("Consultando DENUE por área y actividad...")
    print(f"ENTIDAD={ENTITY_CODE}")
    print(f"MUNICIPIO={MUNICIPALITY_CODE}")
    print(f"CLASE_SCIAN={ACTIVITY_CLASS}")
    print(f"ESTRATO={STRATUM_CODE}")
    print(
        "RANGO="
        f"{INITIAL_RECORD}-{FINAL_RECORD}"
    )

    try:
        response = requests.get(
            request_url,
            timeout=30,
            headers={
                "Accept": "application/json",
                "User-Agent": "OrvexSignalScouting/0.2",
            },
        )
    except requests.RequestException as error:
        print(
            "ERROR_DE_CONEXION="
            f"{type(error).__name__}"
        )
        return 1

    print(f"HTTP_STATUS={response.status_code}")

    if response.status_code != 200:
        print("La API respondió con un estado inesperado.")
        print(response.text[:500])
        return 1

    try:
        data = response.json()
    except ValueError:
        print("ERROR: La respuesta no es JSON válido.")
        print(response.text[:500])
        return 1

    print(f"TIPO_RESPUESTA={type(data).__name__}")

    if not isinstance(data, list):
        print(
            "ERROR: Se esperaba una lista "
            "de establecimientos."
        )
        print(data)
        return 1

    print(f"REGISTROS_RECIBIDOS={len(data)}")

    if not data:
        print(
            "La consulta funcionó, pero no devolvió "
            "establecimientos."
        )
        return 0

    first_record = data[0]

    if not isinstance(first_record, dict):
        print(
            "ERROR: El primer registro no tiene "
            "la estructura esperada."
        )
        return 1

    print("\nCLAVES_DEL_PRIMER_REGISTRO:")

    for key in first_record.keys():
        print(f"- {key}")

    print("\nPRIMEROS_ESTABLECIMIENTOS:")

    for index, record in enumerate(data[:5], start=1):
        print(f"\nESTABLECIMIENTO_{index}")
        print(f"Nombre: {record.get('Nombre', '')}")
        print(
            "Actividad: "
            f"{record.get('Clase_actividad', '')}"
        )
        print(f"Estrato: {record.get('Estrato', '')}")
        print(f"Colonia: {record.get('Colonia', '')}")
        print(f"Ubicación: {record.get('Ubicacion', '')}")
        print(f"Teléfono: {record.get('Telefono', '')}")
        print(
            "Sitio web: "
            f"{record.get('Sitio_internet', '')}"
        )

    return 0


if __name__ == "__main__":
    sys.exit(main())